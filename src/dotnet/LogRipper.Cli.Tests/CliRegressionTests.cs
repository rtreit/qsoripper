...
public class CliRegressionTests
{
    ...
    public async Task Entry_point_shows_command_help_for_log_help()
    {
        ...
        Assert.DoesNotContain("Usage: logripper-cli [options] <command> [arguments]", result.StandardOutput, StringComparison.Ordinal);
    }
    ...
    public async Task Entry_point_rejects_invalid_frequency_before_connecting()
    {
        ...
        Assert.DoesNotContain("Could not connect to LogRipper engine", result.StandardError, StringComparison.Ordinal);
    }
