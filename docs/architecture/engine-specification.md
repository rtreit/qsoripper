...
3. For each remote QSO:
    ...
    d. If matched, apply the configured `ConflictPolicy`:
       - `CONFLICT_POLICY_LAST_WRITE_WINS` — treat values present in the remote record as authoritative and overwrite the corresponding local fields; mark the merged row as `SYNCED`. A field omitted from QRZ's ADIF response does not represent an explicit deletion. Engines MUST preserve populated local `rst_sent` and `rst_received` values when the corresponding remote field is absent, and MUST preserve the local `created_at` and `updated_at` metadata.
       - `CONFLICT_POLICY_FLAG_FOR_REVIEW` — when the local row was locally edited (`sync_status = MODIFIED`), preserve the local fields, set `sync_status = CONFLICT`, and increment the sync result's conflict counter so operators can reconcile manually. When the local row is already `SYNCED`, values present in the remote record win (no conflict), with the same omitted-RST and local-metadata preservation rules as `LAST_WRITE_WINS`.
