...
3. For each remote QSO:
    ...
    d. If matched, apply the configured `ConflictPolicy`:
       - `CONFLICT_POLICY_LAST_WRITE_WINS` - treat values present in the remote record as authoritative and overwrite the corresponding local fields; mark the merged row as `SYNCED`. A field omitted from QRZ's ADIF response does not represent an explicit deletion. Engines MUST preserve every populated local QSO field when the corresponding remote field is absent, including signal reports, contest data, QSL state, enrichment, station snapshots, notes, application fields, split-frequency data, and fields added by later schema versions. Engines MUST always preserve local identity, creation/update metadata, and soft-delete state.
       - `CONFLICT_POLICY_FLAG_FOR_REVIEW` - when the local row was locally edited (`sync_status = MODIFIED`), preserve the local fields, set `sync_status = CONFLICT`, and increment the sync result's conflict counter so operators can reconcile manually. When the local row is already `SYNCED`, values present in the remote record win (no conflict), with the same sparse-field and local-metadata preservation rules as `LAST_WRITE_WINS`.
...
**Concurrent local changes:** Every local writeback that follows a QRZ network request MUST be concurrency-safe. A download merge or conflict-state change may replace the row only if the persisted QSO still matches the snapshot used to make that decision. Upload completion MUST patch only QRZ linkage and sync state onto the current row. Remote-delete completion MUST clear linkage on the current row without restoring a stale tombstone. Operator edits, deletes, and restores made while a QRZ request is in flight always take precedence over a stale sync snapshot.
