...
Engine-backed CW keying defaults to the non-transmitting `null` backend. Configure and probe a WinKeyer with the transmit gate off first, then enable hardware transmission only after `qsoripper cw status` reports the expected port and firmware revision. See [CW keying setup](docs/integrations/cw-keying.md) for the complete safety-first workflow.
